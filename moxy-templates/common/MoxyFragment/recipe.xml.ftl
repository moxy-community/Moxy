...
<recipe>
  ...
  <instantiate from="src/app_package/ui/fragment/BlankFragment.${ktOrJavaExt}.ftl"
    ...
    <#if includeView>
      ...
      <open
          file="${escapeXmlAttribute(srcOut)}/presentation/view/${subpackage}${viewName}.${ktOrJavaExt}"/>
    </#if>
    ...
    <#if includePresenter>
      ...
      <open
          file="${escapeXmlAttribute(srcOut)}/presentation/presenter/${subpackage}${presenterName}.${ktOrJavaExt}"/>
    </#if>
