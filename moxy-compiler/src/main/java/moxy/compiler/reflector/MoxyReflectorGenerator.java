package moxy.compiler.reflector;
...
public class MoxyReflectorGenerator {
  ...
  private MoxyReflectorGenerator() {
  }
  ...
  private static final TypeName MAP_CLASS_TO_OBJECT_TYPE_NAME // Map<Class<*>, Object>
      = ParameterizedTypeName.get(ClassName.get(Map.class), CLASS_WILDCARD_TYPE_NAME,
      TypeName.OBJECT);

  private static final TypeName MAP_CLASS_TO_LIST_OF_OBJECT_TYPE_NAME // Map<Class<*>, List<Object>>
      = ParameterizedTypeName.get(ClassName.get(Map.class), CLASS_WILDCARD_TYPE_NAME,
      LIST_OF_OBJECT_TYPE_NAME);
  ...
  public static JavaFile generate(String destinationPackage,
    ...
    TypeSpec.Builder classBuilder = TypeSpec.classBuilder("MoxyReflector")
        ...
        .addField(MAP_CLASS_TO_OBJECT_TYPE_NAME, "sViewStateProviders", Modifier.PRIVATE,
            Modifier.STATIC)
    ...
    classBuilder.addStaticBlock(
        generateStaticInitializer(presenterClassNames, presentersContainers, strategyClasses,
    ...
    if (destinationPackage.equals(MOXY_REFLECTOR_DEFAULT_PACKAGE)) {
      classBuilder.addMethod(MethodSpec.methodBuilder("getViewState")
          ...
          .addStatement("$1T viewStateProvider = ($1T) sViewStateProviders.get(presenterClass)",
  ...
  private static CodeBlock generateStaticInitializer(List<TypeElement> presenterClassNames,
    ...
    Map<TypeElement, List<TypeElement>> presenterBinders =
        getPresenterBinders(presentersContainers);
    ...
    for (TypeElement presenter : presenterClassNames) {
      ...
      ClassName viewStateProvider = ClassName.get(presenterClassName.packageName(),
          String.join("$", presenterClassName.simpleNames())
              + MvpProcessor.VIEW_STATE_PROVIDER_SUFFIX);
      builder.addStatement("sViewStateProviders.put($T.class, new $T())", presenterClassName,
          viewStateProvider);
    }
    ...
    for (Map.Entry<TypeElement, List<TypeElement>> keyValue : presenterBinders.entrySet()) {
      builder.add("sPresenterBinders.put($T.class, $T.<Object>asList(", keyValue.getKey(),
          Arrays.class);

      boolean isFirst = true;
      for (TypeElement typeElement : keyValue.getValue()) {
        ClassName className = ClassName.get(typeElement);
        String presenterBinderName = String.join("$", className.simpleNames())
   ...
   */
  private static SortedMap<TypeElement, List<TypeElement>> getPresenterBinders(
      List<TypeElement> presentersContainers) {
    ...
    // TreeMap for sorting
    SortedMap<TypeElement, List<TypeElement>> elementListMap =
        new TreeMap<>(TYPE_ELEMENT_COMPARATOR);
